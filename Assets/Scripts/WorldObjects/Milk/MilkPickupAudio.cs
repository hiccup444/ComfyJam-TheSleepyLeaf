...
/// <summary>
/// Plays SFXEvent-driven one-shots when the milk container is picked up or put down.
/// Attach alongside DragItem2D on the milk's draggable child (e.g., "milkFront").
/// </summary>
[DisallowMultipleComponent]
[RequireComponent(typeof(AudioSource))]
public sealed class MilkPickupAudio : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IOnSnappedListener
{
    [Header("SFX Events")]
    [Tooltip("Played when the milk is grabbed / picked up.")]
    [SerializeField] private SFXEvent sfxPickUp;

    [Tooltip("Played when the milk is released / put down.")]
    ...
    {
        // Do not play put-down here; we only play when actually socketed
        if (eventData.button != PointerEventData.InputButton.Left)
            return;
        // Keep _pickupArmed true until OnSnapped.
    }

    public void PlayPickUp() => PlayEvent(sfxPickUp);
    public void PlayPutDown() => PlayEvent(sfxPutDown);

    // Called by DragItem2D after successful snap to a SnapPoint2D
    public void OnSnapped(SnapPoint2D socket)
    {
        if (_pickupArmed)
        {
